...
class SitemapController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($settings->setting_site_sitemap_index_enable == Setting::SITE_SITEMAP_INDEX_DISABLE) {
            ...
            $sitemap_index->addSitemap(route('page.sitemap.blog'));
...
//            $sitemap_page->add(route('page.contact'), $settings->updated_at, '1', $settings->setting_site_sitemap_page_frequency);
    ...
    public function category(Request $request)
    {
        ...
        if ($settings->setting_site_sitemap_category_enable == Setting::SITE_SITEMAP_CATEGORY_DISABLE) {
            ...
            foreach ($categories as $categories_key => $category) {
                if ($category->parent)
                    $sitemap_category->add(route('page.category', ['parent_category_slug' => $category->parent?->category_slug, 'category_slug' => $category->category_slug]), $category->updated_at, '1', $settings->setting_site_sitemap_category_frequency);
                else
                    $sitemap_category->add(route('page.sub_categories', ['category_slug' => $category->category_slug]), $category->updated_at, '1', $settings->setting_site_sitemap_category_frequency);
    ...
    public function blog(Request $request)
    {
        ...
        $sitemap_category->add(route('page.blogs'), $settings->updated_at, '0.8', $settings->setting_site_sitemap_category_frequency);

        $blogs = Blog::all();

        foreach ($blogs as $blog_key => $blog) {

             $sitemap_category->add(route('page.blog', $blog->blog_slug), $blog->updated_at, '0.8', $settings->setting_site_sitemap_category_frequency);
        ...
        if ($settings->setting_site_sitemap_listing_enable == Setting::SITE_SITEMAP_LISTING_DISABLE) {
            ...
            foreach ($items as $items_key => $item) {
                $sitemap_listing->add(
                    ...
                    $item->updated_at, '1', $settings->setting_site_sitemap_listing_frequency);
            }
