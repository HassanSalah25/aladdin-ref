...
use App\Models\ItemHourException;
use App\Models\ItemPhone;
use App\Models\ItemSection;
use App\Models\ItemSectionCollection;
...
//            ->whereIn("ci.category_id", $filter_category_ids);
        ...
        if($search_query)
        {
            $items_query->where('items.item_title', 'LIKE', "%".$search_query."%");
        }
    ...
    public function create(Request $request)
    {
        ...
        return response()->view('backend.admin.item.create',
            compact('all_categories', 'user_id', 'setting_item_max_gallery_photos',
    ...
    public function store(Request $request)
    {
        ...
        $validate_rule = [
            ...
            'item_featured' => 'nullable|numeric',
            'item_title' => 'required',
            'item_description' => 'nullable',
            'city_id' => 'numeric',
            'state_id' => 'numeric',
            'country_id' => 'numeric',
            'item_postal_code' => 'nullable',
            'item_phones.*' => 'nullable',
            'item_website' => 'nullable',
            'item_social_facebook' => 'nullable',
            'item_social_twitter' => 'nullable',
            'item_social_linkedin' => 'nullable',
            'item_social_tiktok' => 'nullable',
            'item_social_pinterest' => 'nullable',
            'item_social_telegram' => 'nullable',
            'item_social_youtube' => 'nullable',
            'item_social_snapchat' => 'nullable',
            'item_youtube_id' => 'nullable',
            'item_type' => 'required|numeric|in:1,2',
            'item_hour_time_zone' => 'nullable',
            'item_hour_show_hours' => 'nullable|numeric|in:1,2',
            'item_social_instagram' => 'nullable',
            'item_social_whatsapp' => 'nullable|numeric',
        ];
        // validate category_ids

        // validate request
        $validate = Validator::make($request->except(['image_gallery','image_image']),$validate_rule);
...
//        }
        ...
        $item_phones = empty($request->item_phones) ? null : $request->item_phones;
        ...
        $item_categories_string = $item_categories_string . " " . Category::find($select_categories)->category_name;
    ...
    public function edit(Item $item)
    {
        ...
        return response()->view('backend.admin.item.edit',
            compact('item_owner', 'all_countries', 'all_states', 'all_cities', 'all_customFields',
                ...
                'time_zone_identifiers', 'item_hours', 'item_hour_exceptions', 'ads'));
    ...
    public function updateItemCategory(Request $request, Item $item)
    {
        ...
        $item->update(['category_id' => $select_categories]);
...
//            'category' => 'required',
            ...
            'item_social_snapchat' => 'nullable',
            'item_youtube_id' => 'nullable',
            'item_type' => 'numeric|in:1,2',
            'item_hour_time_zone' => 'nullable',
            'item_hour_show_hours' => 'nullable|numeric|in:1,2',
            'item_social_instagram' => 'nullable',
            'item_social_whatsapp' => 'nullable|numeric',
            'item_social_email' => 'nullable',
        ];
        ...
        $select_categories = $item->category;
...
//            }
        ...
        $item_website = $request->item_website;
        $item_social_facebook = $request->item_social_facebook;
        $item_social_youtube = $request->item_social_youtube;
        $item_social_twitter = $request->item_social_twitter;
        $item_social_linkedin = $request->item_social_linkedin;
        $item_social_instagram = $request->item_social_instagram;
        $item_social_email = $request->item_social_email;
        $item_social_whatsapp = ltrim($request->item_social_whatsapp, '0');
        ...
        // start saving category string to item_categories_string column of items table
        $item_categories_string = "";
        $item_categories_string = $item_categories_string . " " . $select_categories->category_name;


        $item->category_id = $select_categories->id;
...
//        $item->item_phone = $item_phone;
        ...
        $item->item_social_youtube = $item_social_youtube;
        ...
        $item->item_social_email = $item_social_email;
        ...
        $item->alt_image = $request->alt_image;
        $item->canonical = $request->canonical;


        $item->save();

        $item->itemPhones()->delete();
    ...
    public function itemReviewsIndex(Request $request)
    {
        ...
        {
            $reviews = Review::where('reviewrateable_type','App\Models\Item')
        ...
        else
        {
            ...
            {
                $reviews = Review::where('approved', Item::ITEM_REVIEW_PENDING)
                    ->where('reviewrateable_type','App\Models\Item')
                    ->orderBy('updated_at', 'desc')
                    ->paginate(10);
            }
            ...
            {
                $reviews = Review::where('approved', Item::ITEM_REVIEW_APPROVED)
                    ->where('reviewrateable_type','App\Models\Item')
                    ->orderBy('updated_at', 'desc')
                    ->paginate(10);
            }
    ...
    public function storeItemSectionCollections(Request $request, Item $item, ItemSection $item_section)
    {
         ...
         */
        $request->validate([
            'item_section_collection_collectible_type' => 'required',
    ...
    public function itemAdUpdate(Request $request, Item $item)
    {
        $item->ads()->sync($request->ad_id);
        \Session::flash('flash_message', __('item_section.alert.item-section-ranked-up'));
        \Session::flash('flash_type', 'success');
