...
class LangController extends Controller
{
    ...
    public function index(){
        ...
        return response()->view('backend.admin.lang.index',compact('words'));
        
    }

    public function edit(int $word_id){

        return redirect()->route('admin.lang.show', $word_id);
    }
    public function update(Request $request, int $word_id){
        $request->validate([
            'value' => 'required',
        ]);

        $word = Translation::findOrFail($word_id);
        $word->value = $request->value;
        $word->save();

        \Session::flash('flash_message', __('alert.word-updated'));
        \Session::flash('flash_type', 'success');
    ...
    public function show(int $word_id)
    {
        $word = Translation::findOrFail($word_id);
        return view('backend.admin.lang.show', compact('word'));
