...
class SettingController extends Controller
{
    ...
    public function editSliderPageSetting(Request $request)
    {
        $settings = app('site_global_settings');
        ...
        return response()->view('backend.admin.setting.slider.edit',
            compact('all_page_about_settings'));
    }
    ...
    public function updateSliderPageSetting(Request $request)
    {
        $settings = app('site_global_settings');
        $website_logo_image1 = $request->image1;
        $website_logo_image2 = $request->image2;
        $website_logo_image_name1 = $settings->slider_image_1;
        $website_logo_image_name2 = $settings->slider_image_2;
        if(!empty($website_logo_image1)){

            $currentDate = Carbon::now()->toDateString();

            $website_logo_image_name1 = 'slider-'.$currentDate.'-'.uniqid().'.png';
            $website_logo_image_name2 = 'slider-'.$currentDate.'-'.uniqid().'.png';

            if(!Storage::disk('public')->exists('setting')){
                Storage::disk('public')->makeDirectory('setting');
            }

            $category_image_obj1 = Image::make(base64_decode(preg_replace('#^data:image/\w+;base64,#i', '',$website_logo_image1)))->stream('jpg', 100);
            Storage::disk('public')->put('setting/'.$website_logo_image_name1, $category_image_obj1);

            $category_image_obj = Image::make(base64_decode(preg_replace('#^data:image/\w+;base64,#i', '',$website_logo_image2)))->stream('jpg', 100);
            Storage::disk('public')->put('setting/'.$website_logo_image_name2, $category_image_obj);

        }
        $settings->slider_image_1 = $website_logo_image_name1;
        $settings->slider_image_2 = $website_logo_image_name2;
        ...
        return redirect()->route('admin.settings.page.slider.edit');
    }
...
//                    $current_subscription->subscription_max_free_listing = intval($future_plan->plan_max_free_listing) >= 0 ? $future_plan->plan_max_free_listing : null;
    ...
    public function editItemSetting(Request $request)
    {
         ...
         * Start SEO
         */
        SEOMeta::setTitle(__('theme_alaadin.setting.seo.edit-item', ['site_name' => empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name]));
        SEOMeta::setDescription('');
        SEOMeta::setCanonical(URL::current());
        SEOMeta::addKeyword($settings->setting_site_seo_home_keywords);
        /**
    ...
    public function updateItemSetting(Request $request)
    {
        ...
        \Session::flash('flash_message', __('theme_alaadin.setting.alert.setting-item-updated'));
        \Session::flash('flash_type', 'success');
