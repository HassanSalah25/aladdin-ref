    ...
    <section class="categories pt pb mt-header-height">
        <div class="container">
            ...
            <div class="categories__letters d-flex align-items-center justify-content-center flex-wrap gap-3 mb-md">
                <a  href="{{route('page.categories')}}">الكل</a>
                @foreach($index as $key => $v)
                    <a data-query="{{$v}}" href="{{route('page.categories',['search_keyword'=>request('search_keyword'),'char'=>$v])}}">{{$v}}</a>
                @endforeach
            </div>
                        ...
                        <div class="col d-flex justify-content-center" style="display:table !important">
                            <a
                                ...
                                class="registeration__categories__item" style="display: table-cell; padding: 1.2rem;">
                                    <img src="{{asset('storage/category/' . $category->children->first()?->category_image)}}"
