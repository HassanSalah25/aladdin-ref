...
<section class="categories pt pb mt-header-height">
  <div class="container">
    ...
    <h2 class="fw-bold primary-color mt-lg mb-xl text-center">
      ...
      <form action="{{route('page.sub_categories',['category_slug'=>$category->category_slug])}}" class="row justify-content-center my-5">
          <div class="search">
    ...
    <div class="categories__letters d-flex align-items-center justify-content-center flex-wrap gap-3 mb-md">
      ...
      <a data-query="{{$v}}" href="{{route('page.sub_categories',['search_keyword'=>request('search_keyword'),'category_slug'=>$category->category_slug,'char'=>$v])}}">{{$v}}</a>
      @endforeach
    </div>
