            ...
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="category-slide__item">
                        <img src="{{ asset('frontend/images/placeholder/full-image-slider-placeholder2.jpg') }}" alt="category slider" class="img-fluid" />
                    </div>
                </div>
                    ...
                    <div class="container">
                        <div class="activites__items d-flex align-items-center flex-wrap gap-4">
                            ...
                            <div class="activites__item text-center">
                                <a href="{{ route('page.item', $item->item_slug) }}" class="h5 mb-0 primary-dark-color">{{str_limit($item->item_title,20)  }}</a>
                            </div>
                            @endforeach
                            @endif
--}}{{--
            ...
            <div class="row">
                <div class="col-md-3">
                    ...
                    <div class="cateogries__filters">
                        ...
                        <form method="GET" action="{{ route('page.popular') }}" id="filter_form">

                            <!-- filters  -->
                            <div class="row" style="line-height: 4rem;">
                                <div class="col-12 col-md-12 pl-0">
                                    <label for="filter_state">تصنيف بالمحافظه</label>
                                    <select class=" form-control @error('filter_state') is-invalid @enderror" name="filter_state" id="filter_state" data-live-search="true">

                                        <option value="all" {{ empty($filter_state) ? 'selected' : '' }}>{{ __('prefer_country.all-state') }}</option>

                                        @foreach($states as $all_states_key => $state)

                                        <option value="{{ $state->State }}" {{ $filter_state == $state->State ? 'selected' : '' }}>{{ $state->State }}</option>
                                        @endforeach
                                    </select>
                                    @error('filter_state')
                                ...
                                <div class="col-12 col-md-12 pl-0">
                                    <label for="filter_city">تصنيف بالحي</label>
                                    <select class="form-control @error('filter_city') is-invalid @enderror" name="filter_city" id="filter_city" data-live-search="true">
                                        <option value="0" {{ empty($filter_city) ? 'selected' : '' }}>{{ __('prefer_country.all-city') }}</option>
                                        @foreach($cities as $all_cities_key => $city)
                                        <option value="{{ $city->City }}" {{ $filter_city == $city->City ? 'selected' : '' }}>{{ $city->City }}</option>
                                        @endforeach
                                    </select>
                                    @error('filter_city')
                                    <span class="invalid-tooltip">
                                ...
                                <div class="col-12 col-md-12 pl-0">
                                    ...
                                    <select class=" form-control @error('filter_sort_by') is-invalid @enderror" name="filter_sort_by" id="filter_sort_by">
                            ...
                            <div class="row">
                                <div class="col-12 text-right">
                                    <a class="btn btn-sm btn-outline-primary rounded" href="{{ route('page.popular') }}">
                                        {{ __('theme_alaadin.filter-link-reset-all') }}
                                    </a>
                                    <a class="btn btn-sm btn-primary text-white rounded w-50" id="filter_form_submit">
                ...
                <div class="col-md-9">
                    ...
                    <div class="cateogires__items flex-fill">
                        ...
                        <div class="pagination center-block">
                            {{ $itemsQuery->appends(['filter_city' => request('filter_city'),'filter_sort_by' => request('filter_sort_by'), 'filter_state' => request('filter_state')])->onEachSide(1)->links() }}
                        </div>
                    </div>
                        <!-- start order-area  -->
                        ...
                        <div class="">
                            ...
                            @foreach($itemsQuery as $free_items_key => $item)
                            <!-- <div class="col-lg-6"> -->
                            @include('frontend.partials.paid-item-block',[$page_name = $item->Category])
                            <!-- </div> -->
                            @endforeach
                            @endif
                        </div>
