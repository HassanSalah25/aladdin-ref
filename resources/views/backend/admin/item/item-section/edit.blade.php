        ...
        <div class="col-12">
            ...
            <div class="row mb-2">
                <div class="col-12">
                    ...
                    |
                    @if($item_section->item_section_status == \App\Models\ItemSection::STATUS_DRAFT)
                        <span class="bg-warning pl-1 pr-1 text-white text-sm rounded">
                            {{ __('item_section.item-section-status-draft') }}
                        </span>
                    @elseif($item_section->item_section_status == \App\Models\ItemSection::STATUS_PUBLISHED)
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-4 col-sm-12">
                                ...
                                <select class="custom-select" name="item_section_position">
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_BREADCRUMB }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_BREADCRUMB ? 'selected' : '' }}>{{ __('item_section.position-after-breadcrumb') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_GALLERY }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_GALLERY ? 'selected' : '' }}>{{ __('item_section.position-after-gallery') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_DESCRIPTION }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_DESCRIPTION ? 'selected' : '' }}>{{ __('item_section.position-after-description') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_LOCATION_MAP }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_LOCATION_MAP ? 'selected' : '' }}>{{ __('item_section.position-after-location-map') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_FEATURES }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_FEATURES ? 'selected' : '' }}>{{ __('item_section.position-after-features') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_REVIEWS }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_REVIEWS ? 'selected' : '' }}>{{ __('item_section.position-after-reviews') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_COMMENTS }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_COMMENTS ? 'selected' : '' }}>{{ __('item_section.position-after-comments') }}</option>
                                    <option value="{{ \App\Models\ItemSection::POSITION_AFTER_SHARE }}" {{ (old('item_section_position') ? old('item_section_position') : $item_section->item_section_position) == \App\Models\ItemSection::POSITION_AFTER_SHARE ? 'selected' : '' }}>{{ __('item_section.position-after-share') }}</option>
                                </select>
                            ...
                            <div class="col-md-4 col-sm-12">
                                ...
                                <select class="custom-select" name="item_section_status">
                                    <option value="{{ \App\Models\ItemSection::STATUS_DRAFT }}" {{ (old('item_section_status') ? old('item_section_status') : $item_section->item_section_status) == \App\Models\ItemSection::STATUS_DRAFT ? 'selected' : '' }}>{{ __('item_section.item-section-status-draft') }}</option>
                                    <option value="{{ \App\Models\ItemSection::STATUS_PUBLISHED }}" {{ (old('item_section_status') ? old('item_section_status') : $item_section->item_section_status) == \App\Models\ItemSection::STATUS_PUBLISHED ? 'selected' : '' }}>{{ __('item_section.item-section-status-published') }}</option>
                                </select>
            ...
            @if($all_item_section_collections->count() > 0)
                ...
                @foreach($all_item_section_collections as $all_item_section_collections_key => $collection)
                    <div class="row align-items-center pt-2 mb-2 border-left-info {{ $all_item_section_collections_key%2 == 0 ? 'bg-light' : '' }}">
                        ...
                        @if($collection->item_section_collection_collectible_type == \App\Models\ItemSectionCollection::COLLECTIBLE_TYPE_PRODUCT)

                            @php
                              $find_product = \App\Models\Product::find($collection->item_section_collection_collectible_id);
                            @endphp
                            ...
                            <div class="col-lg-7 col-md-6 col-sm-12">
                                ...
                                |
                                @if($find_product->product_status == \App\Models\Product::STATUS_PENDING)
                                    <span class="bg-warning pl-1 pr-1 text-white text-sm rounded">
                                        {{ __('products.product-status-pending') }}
                                    </span>
                                @elseif($find_product->product_status == \App\Models\Product::STATUS_APPROVED)
                                    <span class="bg-success pl-1 pr-1 text-white text-sm rounded">
                                        {{ __('products.product-status-approved') }}
                                    </span>
                                @elseif($find_product->product_status == \App\Models\Product::STATUS_SUSPEND)
                ...
                @if($available_products->count() > 0)
                    ...
                    <div class="modal-body">
                        <form action="{{ route('admin.items.sections.collections.store', ['item' => $item, 'item_section' => $item_section]) }}" method="POST" id="add-product-form">
                            @csrf
                            <input type="hidden" name="item_section_collection_collectible_type" value="{{ \App\Models\ItemSectionCollection::COLLECTIBLE_TYPE_PRODUCT }}">
                            ...
                            @foreach($available_products as $available_products_key => $available_product)
                                <div class="row align-items-center pt-2 mb-2 border-left-info {{ $available_products_key%2 == 0 ? 'bg-light' : '' }}">
                                    ...
                                    <div class="col-8">
                                        ...
                                        |
                                        @if($available_product->product_status == \App\Models\Product::STATUS_PENDING)
                                            <span class="bg-warning pl-1 pr-1 text-white text-sm rounded">
                                                {{ __('products.product-status-pending') }}
                                            </span>
                                        @elseif($available_product->product_status == \App\Models\Product::STATUS_APPROVED)
                                            <span class="bg-success pl-1 pr-1 text-white text-sm rounded">
                                                {{ __('products.product-status-approved') }}
                                            </span>
                                        @elseif($available_product->product_status == \App\Models\Product::STATUS_SUSPEND)
                                            <span class="bg-danger pl-1 pr-1 text-white text-sm rounded">
                                                {{ __('products.product-status-suspend') }}
                                            </span>
                                        @endif
