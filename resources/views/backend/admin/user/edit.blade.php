...
@section('content')
    ...
    <div class="row bg-white pt-4 pl-3 pr-3 pb-4">
        <div class="col-12">
            <div class="row justify-content-between">
                <div class="col-6">
                    @if($user->user_suspended == \App\Models\User::USER_NOT_SUSPENDED)
                ...
                <div class="col-6 text-right">
                    @if($user->user_suspended == \App\Models\User::USER_NOT_SUSPENDED)
                        <form class="pb-2" action="{{ route('admin.users.suspend', $user) }}" method="POST">
                            ...
                            @method('PUT')
                            <button type="submit"
                                    class="btn btn-danger mb-1">{{ __('backend.user.suspend-account') }}</button>
                        </form>
                    @else
                        <form class="pb-2" action="{{ route('admin.users.unsuspend', $user) }}" method="POST">
                            ...
                            @method('PUT')
                            <button type="submit"
                                    class="btn btn-success mb-1">{{ __('backend.user.un-lock-account') }}</button>
                        </form>
                    @endif
                ...
                <div class="col-12 col-md-10 col-lg-6">
                    <form method="POST" action="{{ route('admin.users.update', $user) }}">
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea id="user_about" class="form-control @error('user_about') is-invalid @enderror"
                                          name="user_about">{{ old('user_about') ? old('user_about') : $user->user_about }}</textarea>
                                @error('user_about')
                                <span class="invalid-tooltip">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-12">
                                <label class="text-black" for="user_type">{{ __('backend.user.user-type') }}</label>
                                <select class="seletpicker form-select" id="user_type" name="user_type">
                                    <option value="1" {{1 == $user->type ? "selectted" : "" }}>{{__('backend.user.user_admin')}}</option>
                                    <option value="2" {{2 == $user->type ? "selectted" : "" }}>{{__('backend.user.user_user')}}</option>
                                </select>
                                @error('user_type')
                                <span class="invalid-tooltip">
                                        <strong>{{ $message }}</strong>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                <label class="text-black"
                                       for="user_prefer_language">{{ __('backend.setting.language.language') }}</label>
                                <select class="custom-select @error('user_prefer_language') is-invalid @enderror"
                                        name="user_prefer_language">
                                    <option value="">{{ __('backend.setting.language.select-language') }}</option>

                                    @foreach(\App\Models\Setting::LANGUAGES as $setting_languages_key => $language)
                                        @if($site_global_settings->settingLanguage->$language == \App\Models\SettingLanguage::LANGUAGE_ENABLE)
                                            <option
                                                value="{{ $setting_languages_key }}" {{ (old('user_prefer_language') ? old('user_prefer_language') : $user->user_prefer_language) == $setting_languages_key ? 'selected' : '' }}>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                <label class="text-black"
                                       for="user_prefer_country_id">{{ __('backend.setting.country') }}</label>
                                <select class="custom-select @error('user_prefer_country_id') is-invalid @enderror"
                                        name="user_prefer_country_id">
                                    ...
                                    @foreach($all_countries as $all_countries_key => $country)
                                        @if($country->country_status == \App\Models\Country::COUNTRY_STATUS_ENABLE)
                                            <option
                                                {{ (old('user_prefer_country_id') ? old('user_prefer_country_id') : $user->user_prefer_country_id) == $country->id ? 'selected' : '' }} value="{{ $country->id }}">{{ $country->country_name }}</option>
                                        @endif
                                    @endforeach
                        ...
                        <div class="row form-group justify-content-between">
                            ...
                            <div class="col-6 text-right" id="show_password">
