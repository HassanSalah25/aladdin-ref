                            ...
                            <div class="row form-group">
                                ...
                                <div class="col-md-12">
                                    <label class="text-black"
                                           for="password_confirmation">{{ __('backend.user.confirm-password') }}</label>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea id="user_about" class="form-control @error('user_about') is-invalid @enderror"
                                          name="user_about">{{ old('user_about') }}</textarea>
                                @error('user_about')
                                <span class="invalid-tooltip">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="seletpicker form-select" id="user_type" name="user_type">
                                    <option
                                        value="1" >{{__('backend.user.user_admin')}}</option>
                                    <option
                                        value="2" >{{__('backend.user.user_user')}}</option>
                                </select>
                                @error('user_type')
                                <span class="invalid-tooltip">
                                        <strong>{{ $message }}</strong>
                                ...
                                <select class="custom-select @error('user_prefer_language') is-invalid @enderror"
                                    ...
                                    @foreach(\App\Models\Setting::LANGUAGES as $setting_languages_key => $language)
                                        @if($site_global_settings->settingLanguage->$language == \App\Models\SettingLanguage::LANGUAGE_ENABLE)
                                            <option
                                                value="{{ $setting_languages_key }}" {{ old('user_prefer_language') == $setting_languages_key ? 'selected' : '' }}>
                                ...
                                <select class="custom-select @error('user_prefer_country_id') is-invalid @enderror"
                                    ...
                                    @foreach($all_countries as $all_countries_key => $country)
                                        @if($country->country_status == \App\Models\Country::COUNTRY_STATUS_ENABLE)
                                            <option
                                                {{ old('user_prefer_country_id') == $country->id ? 'selected' : '' }} value="{{ $country->id }}">{{ $country->country_name }}</option>
                                        @endif
                                    @endforeach
                                    ...
                                    <div class="col-8">
                                        ...
                                        @if(empty($user_admin->user_image))
                                            <img id="image_preview"
                                                 src="{{ asset('backend/images/placeholder/profile-' . intval(rand(0,9)) . '.webp') }}"
                                                 class="img-responsive">
                                        @else
                                            <img id="image_preview"
                                                 src="{{ Storage::disk('public')->url('user/'. $user_admin->user_image) }}"
                                                 class="img-responsive">
                                        @endif
