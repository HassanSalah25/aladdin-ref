...
@section('content')
    ...
    <div class="row bg-white pt-4 pl-3 pr-3 pb-4">
        <div class="col-12">
            ...
            <div class="row mt-2">
                <div class="col-12">
                    @if($import_csv_data->import_csv_data_for_model == \App\Models\ImportCsvData::IMPORT_CSV_FOR_MODEL_LISTING)
                        ...
                        <div class="row align-items-center border-left-info pt-1 pb-1 mb-2 {{ $import_csv_data_sample_header_key%2 == 0 ? 'bg-light' : '' }}">
                            ...
                            <div class="col-12 col-md-3">
                                <select class="custom-select select_csv_data_columns" name="csv_data_columns[]">
                                    @foreach(\App\Models\ImportItemData::DATA_COLUMNS as $data_column_key => $data_column_value)
                                        <option {{ $import_csv_data_sample_header_key == $data_column_key ? 'selected' : '' }} value="{{ $data_column_key }}">
                                            {{ __('theme_alaadin.importer.basic') . ': ' }}
                                    ...
                                    @foreach($available_custom_fields_ids as $available_custom_fields_ids_key => $available_custom_fields_id)
                                        ...
                                        @php
                                        $custom_field_exist = \App\Models\CustomField::find($available_custom_fields_id);
                                        @endphp

                                        @if($custom_field_exist)
                                            <option value="{{ 'custom_field_' . $custom_field_exist->id }}">
                                                {{ __('theme_alaadin.importer.custom-field') . ': ' . $custom_field_exist->custom_field_name }}
                                            </option>
                                        @endif

                                    @endforeach

                                    <option value="{{ \App\Models\ImportItemData::DATA_COLUMNS_DO_NOT_PARSE }}">{{ __('importer_csv.import-listing-do-not-parse') }}</option>
                                </select>
                            </div>
                        </div>
                    @endforeach
